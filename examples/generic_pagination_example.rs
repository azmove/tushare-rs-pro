...
use tushare_api::{Api, TushareClient, TushareEntityList, request};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "First stock via items(): {} - {}",
        first_stock.ts_code, first_stock.name
    );
    ...
    println!(
        "✅ New way: let stocks: TushareEntityList<Stock> = client.call_api_as<Stock>(request).await?;"
    );
